...
public class InputsManager{
    final static String[][] kelly={{"kelly5","1800"},
    ...
    public static void generateDepotEdges(Inputs inputs){
        ...
        for(Integer i = 1; i < CNodes.length; i++){
            CNode iCNode = CNodes[i];
            System.out.println("nodo nro:"+i.toString());
    ...
    /**
     * Lee instancias de problemas fueron hardcodeadas en arrays para evitar leer de hrdf
     * @param depots datos de los clientes, coordenadas y demanda
     * @param vehiculec capacidad del vehiculo, solo se resuelven instancias de 1 vehiculo
     * @param ndepots numero total de depots
     * @return iput al problema
     */
    public static Inputs readInputs(String[] depots, String vehiculec,Integer ndepots) {
        Inputs inputs = null;
        System.out.println("cantidad de depots:"+ndepots.toString());
        inputs = new Inputs(ndepots);
        Integer k = 0;
        int i=0;
        while(i<depots.length){
            Float x = Float.parseFloat(depots[i++]); // Coordenada x
            Float y = Float.parseFloat(depots[i++]);
            Float demand = Float.parseFloat(depots[i++]);
            CNode cNode = new CNode(k, x, y, demand);
            System.out.println("nodro nro:"+k.toString()+" x:"+x.toString()+" y:"+y.toString()+" demand:"+demand.toString());
            inputs.getCNodes()[k] = cNode;
            k++;
        };
         inputs.setVehCap(Float.parseFloat(vehiculec));
        return inputs;
    }

    public static Inputs readInputsK() {
        Inputs inputs= InputsManager.readInputs(kelly[1], kelly[0][1],Integer.parseInt(kelly[3][0]));
